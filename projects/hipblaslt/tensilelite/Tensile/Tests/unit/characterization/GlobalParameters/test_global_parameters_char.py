...
``assignGlobalParameters`` (config merge + env + version branches),
...
def _set_rocm_version(monkeypatch, version="6.2.0"):
...
def test_assign_global_parameters_rejects_invalid_version(
    isolate_globals, isa_info_map, monkeypatch
):
    _set_rocm_version(monkeypatch, "development")
