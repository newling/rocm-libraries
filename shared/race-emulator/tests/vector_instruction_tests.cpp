...
TEST(Instructions, InstructionTestZero) {
  Workgroup wg({.vgprCount = 10, .sgprCount = 10, .waveSize = WaveSize{16}});
  auto &wave = wg.getWave(0);
  tryExecute(wave, "v_mov_b32_e32 v1, 17");
  EXPECT_EQ(wave.getVgpr(1, 0), 17);

  tryExecute(wave, "s_mov_b32 s3, 18");
  tryExecute(wave, "v_mov_b32_e32 v1, s3");
  EXPECT_EQ(wave.getVgpr(1, 0), 18);

  wave.setVgpr64(0, /* lane */ 3, 2);
  wave.setVgpr64(2, /* lane */ 3, 137);
  // 137 + (2 << 3) = 137 + 16 = 153
  tryExecute(wave, "v_lshl_add_u64 v[2:3], v[0:1], 3, v[2:3]");
  EXPECT_EQ(wave.getVgpr64(2, 3), 153);

  // Let's now test v_add_f64.
  wave.setVgpr64(4, /* lane */ 5, std::bit_cast<uint64_t>(2.5));
  wave.setVgpr64(6, /* lane */ 5, std::bit_cast<uint64_t>(3.5));
  tryExecute(wave, "v_add_f64 v[4:5], v[6:7], v[4:5]");
  double result = std::bit_cast<double>(wave.getVgpr64(4, 5));
  EXPECT_EQ(result, 6.0);
}

TEST(Instructions, MixedOperandMath) {
  Workgroup wg({.vgprCount = 16, .sgprCount = 16, .waveSize = WaveSize{32}});
  auto &wave = wg.getWave(0);

  // Setup: s0 = 10.0f (stored as bits), v2 = 5.0f (all lanes)
  uint32_t float10bits = std::bit_cast<uint32_t>(10.0f);
  wave.setSgpr(0, float10bits);
  EXPECT_EQ(wave.getSgpr(0), float10bits);

  for (int i = 0; i < 32; ++i) {
    wave.setVgpr(2, /*lane*/ i, std::bit_cast<uint32_t>(5.0f));
  }

  // 2. Register + Literal
  // v_mul_f32 v1, s0, 2.0 -> v1 = 10.0 * 2.0 = 20.0
  tryExecute(wave, "v_mul_f32_e32 v1, s0, 2.0");
  float res2 = std::bit_cast<float>(wave.getVgpr(1, /*lane*/ 15));
  EXPECT_FLOAT_EQ(res2, 20.0f);

  // 1. Literal + Literal (Immediate math)
  // v_add_f32 v0, 1.5, 2.5 -> v0 should be 4.0
  tryExecute(wave, "v_add_f32_e32 v0, 1.5, 2.5");
  float res1 = std::bit_cast<float>(wave.getVgpr(0, /*lane*/ 0));
  EXPECT_FLOAT_EQ(res1, 4.0f);

  // 3. VGPR + SGPR
  // v_add_f32 v3, v1, s0 -> 20.0 + 10.0 = 30.0
  tryExecute(wave, "v_add_f32_e32 v3, v1, s0");
  float res3 = std::bit_cast<float>(wave.getVgpr(3, /*lane*/ 31));
...
TEST(Instructions, LaneIndependence) {
  ...
  wave.setVgpr(0, /*lane*/ 0, 10);
  wave.setVgpr(0, /*lane*/ 1, 20);
  wave.setVgpr(0, /*lane*/ 2, 30);
  wave.setVgpr(0, /*lane*/ 3, 40);

  // Execute a shift operation: v1 = (v0 << 1) | 0
  tryExecute(wave, "v_lshl_or_b32 v1, v0, 1, 0");

  EXPECT_EQ(wave.getVgpr(1, /*lane*/ 0), 20);
  EXPECT_EQ(wave.getVgpr(1, /*lane*/ 1), 40);
  EXPECT_EQ(wave.getVgpr(1, /*lane*/ 2), 60);
  EXPECT_EQ(wave.getVgpr(1, /*lane*/ 3), 80);
}

TEST(Instructions, Registers64Bit) {
  Workgroup wg({.vgprCount = 4, .sgprCount = 4, .waveSize = WaveSize{1}});
  auto &wave = wg.getWave(0);

  uint64_t bigValue = 0x123456789ABCDEF0;

  // 1. Set via helper
  wave.setVgpr64(0, /*lane*/ 0, bigValue);

  // 2. Verify individual 32-bit registers (Little Endian)
  // v0 = Low 32 bits, v1 = High 32 bits
  EXPECT_EQ(wave.getVgpr(0, /*lane*/ 0), 0x9ABCDEF0);
  EXPECT_EQ(wave.getVgpr(1, /*lane*/ 0), 0x12345678);

  // 3. Move via instruction
  tryExecute(wave, "v_mov_b32_e32 v2, v0"); // v2 = low
  tryExecute(wave, "v_mov_b32_e32 v3, v1"); // v3 = high

  // 4. Reconstruct 64-bit from v[2:3]
  uint64_t reconstructed = wave.getVgpr64(2, /*lane*/ 0);
...
TEST(Instructions, Conversions) {
  ...
  wave.setVgpr(0, /*lane*/ 0, std::bit_cast<uint32_t>(input));

  // v[2:3] = double(v0)
  tryExecute(wave, "v_cvt_f64_f32_e32 v[2:3], v0");

  double result = std::bit_cast<double>(wave.getVgpr64(2, /*lane*/ 0));
  // Allow small epsilon due to f32->f64 precision expansion
  EXPECT_NEAR(result, 3.14159, 1e-6);

  // Double -> Float
  wave.setVgpr64(2, /*lane*/ 0, std::bit_cast<uint64_t>(12345.6789));

  // v1 = float(v[2:3])
  tryExecute(wave, "v_cvt_f32_f64_e32 v1, v[2:3]");

  float resF = std::bit_cast<float>(wave.getVgpr(1, /*lane*/ 0));
...
TEST(Instructions, VLshlOrB32) {
  ...
  wave.setVgpr(0, /*lane*/ 0, 10);
  wave.setVgpr(1, /*lane*/ 0, 5);
  // v2 = (v0 << 2) | v1
  tryExecute(wave, "v_lshl_or_b32 v2, v0, 2, v1");
  EXPECT_EQ(wave.getVgpr(2, /*lane*/ 0), 45);

  // 2. Test Shift Masking (Shift > 31)
  // Shift 33 is equivalent to shift 1 (33 & 31 = 1).
  // (10 << 1) | 0 = 20
  tryExecute(wave, "v_lshl_or_b32 v3, v0, 33, 0");
  EXPECT_EQ(wave.getVgpr(3, /*lane*/ 0), 20);
  ...
  // v2 = (v0 << s0) | v1
  // (10 << 3) | 5 = 80 | 5 = 85
  tryExecute(wave, "v_lshl_or_b32 v2, v0, s0, v1");
  EXPECT_EQ(wave.getVgpr(2, /*lane*/ 0), 85);
}

TEST(Instructions, VCmpGtI32E32) {
  // Example: v_cmp_gt_i32_e32 vcc, s5, v0
  Workgroup wg({.vgprCount = 2, .sgprCount = 6, .waveSize = WaveSize{64}});
  auto &wave = wg.getWave(0);
  wave.setSgpr(2, 25); // s2 = 25
  // set v1 iota from 0:
  for (int lane = 0; lane < wave.getWaveSize(); ++lane) {
    wave.setVgpr(1, lane, 100);
  }
  wave.setVgpr(1, 20, 24);
  wave.setVgpr(1, 40, 25); // 25 > 25 ? no.
  wave.setVgpr(1, 50, 7);
  uint64_t expected = (uint64_t(1) << 50) |
                      (uint64_t(1) << 20); // lanes 20 and 50 should be set.

  tryExecute(wave, "v_cmp_gt_i32_e32 vcc, s2, v1");
  EXPECT_EQ(wave.getVccU64(), expected);

  tryExecute(wave, "v_cmp_gt_i32_e32 vcc, 0, 1");
  EXPECT_EQ(wave.getVccU64(), 0);

  tryExecute(wave, "v_cmp_gt_i32_e32 vcc, 1, 0");
  EXPECT_EQ(wave.getVccU64(), uint64_t(-1));
}

TEST(Instructions, VReadFirstLane) {
  // Setup: 1 VGPR, 1 SGPR, WaveSize 64
  Workgroup wg({.vgprCount = 1, .sgprCount = 1, .waveSize = WaveSize{64}});
  auto &wave = wg.getWave(0);

  // 1. Initialize v0 such that Lane N contains the value N.
  // This acts as our "Lane ID" map.
  for (int i = 0; i < 64; ++i) {
    wave.setVgpr(0, i, static_cast<uint32_t>(i));
  }

  // Case 1: Lowest bit set (Lane 0)
  // EXEC = ...0001
  wave.setExecU64(1);
  tryExecute(wave, "v_readfirstlane_b32 s0, v0");
  // Should read from Lane 0 (value 0)
  EXPECT_EQ(wave.getSgpr(0), 0);

  // Case 2: A middle bit set (Lane 4)
  // EXEC = ...00010000
  wave.setExecU64(1ULL << 4);
  tryExecute(wave, "v_readfirstlane_b32 s0, v0");
  // Should read from Lane 4 (value 4)
  EXPECT_EQ(wave.getSgpr(0), 4);

  // Case 3: Multiple bits set (Lanes 10 and 20 active)
  // Spec says: "Lowest active lane"
  wave.setExecU64((1ULL << 10) | (1ULL << 20));
  tryExecute(wave, "v_readfirstlane_b32 s0, v0");
  // Should pick Lane 10, not 20
  EXPECT_EQ(wave.getSgpr(0), 10);

  // Case 4: High bit set (Lane 63)
  wave.setExecU64(1ULL << 63);
  tryExecute(wave, "v_readfirstlane_b32 s0, v0");
  EXPECT_EQ(wave.getSgpr(0), 63);
  ...
  tryExecute(wave, "v_readfirstlane_b32 s0, v0");
  EXPECT_EQ(wave.getSgpr(0), 0xDEADBEEF);
}
...
TEST(Instructions, VAshrRevI32_LLVMExamples) {
  ...
  tryExecute(wave, "v_ashrrev_i32_e32 v0, 1, 4");
  EXPECT_EQ(static_cast<int32_t>(wave.getVgpr(0, 0)), 2);

  // LLVM Example 2: result = ashr i32 4, 2
  // Yields: 1
  tryExecute(wave, "v_ashrrev_i32_e32 v0, 2, 4");
  EXPECT_EQ(static_cast<int32_t>(wave.getVgpr(0, 0)), 1);

  // LLVM Example 3: result = ashr i8 -2, 1
  // -2 (0xFFFFFFFE) >> 1 = -1 (0xFFFFFFFF), sign bit preserved.
  wave.setVgpr(1, 0, -2); // Load -2 into v1
  tryExecute(wave, "v_ashrrev_i32_e32 v0, 1, v1");
  EXPECT_EQ(static_cast<int32_t>(wave.getVgpr(0, 0)), -1);
  ...
  // Shift 33 -> 33 & 0x1F = 1.
  // 4 >> 1 = 2.
  tryExecute(wave, "v_ashrrev_i32_e32 v0, 33, 4");
  EXPECT_EQ(static_cast<int32_t>(wave.getVgpr(0, 0)), 2);
}
...
TEST(Instructions, VBfeU32) {
  ...
  wave.setVgpr(0, 0, 0xABCD1234);

  // Case 1: Extract "3" (Nibble at bit 4)
  // Offset = 4, Width = 4
  wave.setVgpr(1, 0, 4); // Offset
  wave.setVgpr(2, 0, 4); // Width

  tryExecute(wave, "v_bfe_u32 v3, v0, v1, v2");
  EXPECT_EQ(wave.getVgpr(3, 0), 3);

  // Case 2: Extract "12" (Byte at bit 8)
  // Offset = 8, Width = 8
  // Result should be 0x12
  tryExecute(wave, "v_bfe_u32 v3, v0, 8, 8"); // Using literals for offset/width
  EXPECT_EQ(wave.getVgpr(3, 0), 0x12);

  // Case 3: Zero Width -> Result 0
  tryExecute(wave, "v_bfe_u32 v3, v0, 5, 0");
  EXPECT_EQ(wave.getVgpr(3, 0), 0);
}
TEST(Instructions, VAdd3U32) {
  ...
  tryExecute(wave, "v_add3_u32 v0, v1, v2, s0");

  EXPECT_EQ(wave.getVgpr(0, 0), 35);

  // Case: Overflow wrapping
  // Max + 1 + 1 = 1
  wave.setVgpr(1, 0, 0xFFFFFFFF);
  wave.setVgpr(2, 0, 1);
  wave.setSgpr(0, 1);

  tryExecute(wave, "v_add3_u32 v0, v1, v2, s0");
  EXPECT_EQ(wave.getVgpr(0, 0), 1);
}

TEST(Instructions, VCmpU32Sdwa_Unified) {
  Workgroup wg({.vgprCount = 4, .sgprCount = 2, .waveSize = WaveSize{1}});
  auto &wave = wg.getWave(0);

  // Data Setup
  // v1: High=0xAAAA, Low=0x1234 -> 0xAAAA1234
  // v2: High=0xBBBB, Low=0x1234 -> 0xBBBB1234
  wave.setVgpr(1, 0, 0xAAAA1234);
  wave.setVgpr(2, 0, 0xBBBB1234);
  ...
  tryExecute(
      wave, "v_cmp_eq_u32_sdwa s[0:1], v1, v2 src0_sel:WORD_0 src1_sel:WORD_0");
  EXPECT_EQ(wave.getSgpr64(0), 1ULL);

  // Case 2: Compare Upper Words (WORD_1)
  // v1.WORD_1 (0xAAAA) == v2.WORD_1 (0xBBBB) -> NO MATCH
  wave.setSgpr64(0, 0); // Reset Dest
  tryExecute(
      wave, "v_cmp_eq_u32_sdwa s[0:1], v1, v2 src0_sel:WORD_1 src1_sel:WORD_1");
  EXPECT_EQ(wave.getSgpr64(0), 0ULL);

  // Case 3: Mixed Selectors (Byte vs Word)
  // v1 = 0xAAAA1234
  // Check if BYTE_2 (0x12) matches the value in v3
  wave.setVgpr(3, 0, 0x00000012);
  wave.setSgpr64(0, 0);
  ...
  tryExecute(wave, "v_cmp_eq_u32_sdwa s[0:1], v1, v2");
  EXPECT_EQ(wave.getSgpr64(0), 0ULL);
}
...
TEST(Instructions, VCndMask_MultiLane) {
  ...
  Workgroup wg({.vgprCount = 3, .sgprCount = 2, .waveSize = WaveSize{2}});
  auto &wave = wg.getWave(0);

  // 1. Initialize Inputs per Lane
  // Lane 0: v1=100, v2=200
  wave.setVgpr(1, 0, 100);
  wave.setVgpr(2, 0, 200);

  // Lane 1: v1=100, v2=200
  wave.setVgpr(1, 1, 100);
  wave.setVgpr(2, 1, 200);
  ...
  wave.setSgpr64(0, 0x2);

  // 3. Execute
  // v0 = VCC ? v2 : v1
  tryExecute(wave, "v_cndmask_b32_e32 v0, v1, v2, s[0:1]");

  // 4. Verify Results

  // Lane 0: VCC bit 0 was 0. Should have picked v1 (100).
  EXPECT_EQ(wave.getVgpr(0, 0), 100);

  // Lane 1: VCC bit 1 was 1. Should have picked v2 (200).
  EXPECT_EQ(wave.getVgpr(0, 1), 200);
}

TEST(Instructions, VCndMask_RespectsExec) {
  Workgroup wg({.vgprCount = 3, .sgprCount = 2, .waveSize = WaveSize{64}});
  auto &wave = wg.getWave(0);
  ...
  wave.setExecU64(0x1); // Only bit 0 is set
  ...
  EXPECT_EQ(wave.getVgpr(0, 0), 10); // Updated
  EXPECT_EQ(wave.getVgpr(0, 1), 99); // Preserved! (If this is 10, EXEC failed)
}
...
TEST(Instructions, VAddLshlU32) {
  ...
  //        = 30 << 2
  //        = 120
  wave.setVgpr(1, 0, 10); // Src0
  wave.setVgpr(2, 0, 20); // Src1
  wave.setVgpr(3, 0, 2);  // Shift

  tryExecute(wave, "v_add_lshl_u32 v0, v1, v2, v3");
  EXPECT_EQ(wave.getVgpr(0, 0), 120);
  ...
  wave.setVgpr(1, 0, 1);
  wave.setVgpr(2, 0, 1);
  wave.setVgpr(3, 0, 33); // 0x21, 0x21 & 0x1F = 1

  tryExecute(wave, "v_add_lshl_u32 v0, v1, v2, v3");
  EXPECT_EQ(wave.getVgpr(0, 0), 4);
}

TEST(Instructions, VOrB32Sdwa_Foundation) {
  // Setup: 4 VGPRs, 4 SGPRs, Wave32
  Workgroup wg({.vgprCount = 4, .sgprCount = 4, .waveSize = WaveSize{32}});
  auto &wave = wg.getWave(0);

  // Initial State
  // v1 = 0x11112222
  // s0 = 0x00003333
  wave.setVgpr(1, 0, 0x11112222);
  wave.setSgpr(0, 0x00003333);
  ...
  tryExecute(wave, "v_or_b32_sdwa v1, s0, v1 dst_sel:WORD_1 "
                   "dst_unused:UNUSED_PAD src0_sel:DWORD src1_sel:DWORD");
  EXPECT_EQ(wave.getVgpr(1, 0), 0x33330000);
  ...
  EXPECT_EQ(wave.getVgpr(1, 0), 0x33332222);
}

TEST(Instructions, VOrB32Sdwa_LLVM_Patterns) {
  Workgroup wg({.vgprCount = 64, .sgprCount = 4, .waveSize = WaveSize{1}});
  auto &wave = wg.getWave(0);
  ...
  wave.setVgpr(22, 0, valA);
  wave.setVgpr(3, 0, valB);

  // Exp: A | (B & 0xFF) = 0x12345678 | 0x90 = 0x123456F8
  tryExecute(wave, "v_or_b32_sdwa v3, v22, v3 dst_sel:DWORD "
                   "dst_unused:UNUSED_PAD src0_sel:DWORD src1_sel:BYTE_0");
  EXPECT_EQ(wave.getVgpr(3, 0), 0x123456F8);
  ...
  wave.setVgpr(3, 0, valA); // src0
  wave.setVgpr(2, 0, valB); // src1

  // Exp: (A & 0xFFFF) | B = 0x5678 | 0xABCDEF90
  //                       =          0xABCDEF90
  //                                  0x00005678
  //                                  0xABCDFFF8
  tryExecute(wave, "v_or_b32_sdwa v2, v3, v2 dst_sel:DWORD "
                   "dst_unused:UNUSED_PAD src0_sel:WORD_0 src1_sel:DWORD");
  EXPECT_EQ(wave.getVgpr(2, 0), 0xABCDFFF8);

  // Pattern 3: Tied Operand + High Word Dest (The "Tied Trap")
  // v_or_b32_sdwa v4, v4, v3 dst_sel:WORD_1 ... src0_sel:DWORD src1_sel:BYTE_0
  wave.setVgpr(4, 0, valA); // src0 AND dst (old)
  wave.setVgpr(3, 0, valB); // src1
  ...
  tryExecute(wave, "v_or_b32_sdwa v4, v4, v3 dst_sel:WORD_1 "
                   "dst_unused:UNUSED_PAD src0_sel:DWORD src1_sel:BYTE_0");
  EXPECT_EQ(wave.getVgpr(4, 0), 0x56F80000);

  // Pattern 4: Src0 is BYTE_0 (Reverse of Pattern 1)
  // v_or_b32_sdwa v0, v44, v62 ... src0_sel:BYTE_0 src1_sel:DWORD
  wave.setVgpr(44, 0, valA);
  wave.setVgpr(62, 0, valB);

  // Exp: (A & 0xFF) | B = 0x78 | 0xABCDEF90 = 0xABCDEFF8
  tryExecute(wave, "v_or_b32_sdwa v0, v44, v62 dst_sel:DWORD "
                   "dst_unused:UNUSED_PAD src0_sel:BYTE_0 src1_sel:DWORD");
  EXPECT_EQ(wave.getVgpr(0, 0), 0xABCDEFF8);
}

// Helper to verify packed results easily
void expectPackedFloats(const Wave &wave, int dstIdx, float expectedLo,
                        float expectedHi) {
  uint64_t raw = wave.getVgpr64(dstIdx, 0);
...
TEST(Instructions, V_PK_MUL_F32_OpSel_Logic) {
  Workgroup wg({.vgprCount = 6, .sgprCount = 0, .waveSize = WaveSize{2}});
  auto &wave = wg.getWave(0);
  ...
  wave.setVgpr64(0, 0, pack0);
  wave.setVgpr64(2, 0, pack1);
  ...
  // Result: Lo = 2 * 3 = 6
  //         Hi = 10 * 100 = 1000
  tryExecute(wave, "v_pk_mul_f32 v[4:5], v[0:1], v[2:3]");
  expectPackedFloats(wave, 4, 6.0f, 1000.0f);
  ...
  tryExecute(wave, "v_pk_mul_f32 v[4:5], v[0:1], v[2:3] op_sel_hi:[0,1]");
  expectPackedFloats(wave, 4, 6.0f, 200.0f);

  // Case 3: Mixed Selection (Cross Multiply)
  // op_sel:[1,0] -> "For Low calc, use Src0.Hi (1) and Src1.Lo (0)"
  // Result: Lo = 10 * 3 = 30
  //         Hi = 10 * 100 = 1000 (Default)
  tryExecute(wave, "v_pk_mul_f32 v[4:5], v[0:1], v[2:3] op_sel:[1,0]");
  expectPackedFloats(wave, 4, 30.0f, 1000.0f);
  ...
  //         Hi = 2 * 3 = 6
  tryExecute(
      wave, "v_pk_mul_f32 v[4:5], v[0:1], v[2:3] op_sel:[1,1] op_sel_hi:[0,0]");
  expectPackedFloats(wave, 4, 1000.0f, 6.0f);
}

TEST(Instructions, V_PK_MUL_F32_Negation) {
  Workgroup wg({.vgprCount = 6, .sgprCount = 0, .waveSize = WaveSize{64}});
  auto &wave = wg.getWave(0);
  ...
  wave.setVgpr64(0, 0, pack);

  pack = (static_cast<uint64_t>(std::bit_cast<uint32_t>(3.0f)) << 32) |
         std::bit_cast<uint32_t>(3.0f);
  wave.setVgpr64(2, 0, pack);

  // Case: Negate Low Source 0
  // neg_lo:[1,0] -> Negate Src0 for the Low calculation only
  // Lo = (-2.0) * 3.0 = -6.0
  // Hi = 2.0 * 3.0 = 6.0 (Default)
  tryExecute(wave, "v_pk_mul_f32 v[4:5], v[0:1], v[2:3] neg_lo:[1,0]");
  expectPackedFloats(wave, 4, -6.0f, 6.0f);

  // Case: Negate High Source 1
  // neg_hi:[0,1] -> Negate Src1 for the High calculation only
  // Lo = 2.0 * 3.0 = 6.0
  // Hi = 2.0 * (-3.0) = -6.0
  tryExecute(wave, "v_pk_mul_f32 v[4:5], v[0:1], v[2:3] neg_hi:[0,1]");
  expectPackedFloats(wave, 4, 6.0f, -6.0f);
}

TEST(Instructions, V_FMAC_F32) {
  Workgroup wg({.vgprCount = 4, .sgprCount = 0, .waveSize = WaveSize{64}});
  auto &wave = wg.getWave(0);

  // Setup Accumulator (Dst/v0) = 10.0
  wave.setVgpr(0, 0, std::bit_cast<uint32_t>(10.0f));

  // Setup Src0 (v1) = 2.0
  wave.setVgpr(1, 0, std::bit_cast<uint32_t>(2.0f));

  // Setup Src1 (v2) = 3.0
  wave.setVgpr(2, 0, std::bit_cast<uint32_t>(3.0f));

  // Execute: v0 = v1 * v2 + v0
  // Expected: (2.0 * 3.0) + 10.0 = 16.0
  tryExecute(wave, "v_fmac_f32 v0, v1, v2");

  float result = std::bit_cast<float>(wave.getVgpr(0, 0));
...
TEST(Instructions, V_CMPX_EQ_U32_BasicLogic) {
  ...
  wave.setExecU64(0xFFFFFFFFFFFFFFFF);

  // Setup Inputs
  // v0: [0, 1, 2, 3, 0, 1, 2, 3, ...]
  // v1: 2 (Scalar comparison value)
  for (int i = 0; i < 64; ++i) {
    wave.setVgpr(0, i, i % 4);
    wave.setVgpr(1, i, 2);
  }
  ...
  tryExecute(wave, "v_cmpx_eq_u32 vcc, v0, v1");
  ...
  EXPECT_EQ(wave.getExecU64(), expectedMask);

  // VCC should also reflect the new EXEC mask
  EXPECT_EQ(wave.getVccU64(), expectedMask);
}
...
TEST(Instructions, V_CMPX_EQ_U32_ExecMaskingBehavior) {
  ...
  wave.setExecU64(0x00000000FFFFFFFF);
  ...
  EXPECT_EQ(wave.getExecU64(), 0x00000000FFFFFFFFULL)
      << "V_CMPX must perform (Result & Old_EXEC). Threads already masked off "
         "must stay dead.";

  EXPECT_EQ(wave.getVccU64(), 0x00000000FFFFFFFFULL);
}
...
TEST(Instructions, V_CMPX_EQ_U32_WithScalars) {
  ...
  for (int i = 0; i < 64; ++i) {
    ...
    wave.setVgpr(0, i, (i % 2 == 0) ? 42 : 99);
  }

  tryExecute(wave, "v_cmpx_eq_u32 vcc, s0, v0");

  // Expect: 0101 0101 ... (0x5)
  // Bit 0 is 1 because Lane 0 matches.
  EXPECT_EQ(wave.getExecU64(), 0x5555555555555555ULL);
}

TEST(Instructions, V_ADD_CO_U32) {
  Workgroup wg({.vgprCount = 3, .sgprCount = 0, .waveSize = WaveSize{64}});
  auto &wave = wg.getWave(0);
  ...
  wave.setVgpr(0, 1, 0xFFFFFFFF);
  wave.setVgpr(1, 1, 1);

  // Lane 2: Boundary Case (UINT_MAX + UINT_MAX = -2)
  wave.setVgpr(0, 2, 0xFFFFFFFF);
  wave.setVgpr(1, 2, 0xFFFFFFFF);

  // Ensure VCC is clean
  wave.setVccU64(0);

  // Execute: v2 = v0 + v1, writes carry to vcc
  tryExecute(wave, "v_add_co_u32 v2, vcc, v0, v1");

  // Check Lane 0
  EXPECT_EQ(wave.getVgpr(2, 0), 30);

  // Check Lane 1
  EXPECT_EQ(wave.getVgpr(2, 1), 0);

  // Check Lane 2
  EXPECT_EQ(wave.getVgpr(2, 2), 0xFFFFFFFE);
  ...
  EXPECT_EQ(wave.getVccU64() & 0x7, 6);
}
...
TEST(Instructions, V_ADDC_CO_U32) {
  ...
  wave.setVccU64(0b110);

  // Execute: v2 = v0 + v1 + vcc(in), writes carry to vcc(out)
  tryExecute(wave, "v_addc_co_u32 v2, vcc, v0, v1, vcc");

  // Check Lane 0
  EXPECT_EQ(wave.getVgpr(2, 0), 30);

  // Check Lane 1
  EXPECT_EQ(wave.getVgpr(2, 1), 31);

  // Check Lane 2
  EXPECT_EQ(wave.getVgpr(2, 2), 0);
  ...
  EXPECT_EQ(wave.getVccU64() & 0x7, 4);
}
...
TEST(Instructions, V_LSHL_ADD_U32) {
  ...
  // (5 << 2) + 10 = 20 + 10 = 30
  wave.setVgpr(0, 0, 5);  // Base
  wave.setVgpr(1, 0, 2);  // Shift
  wave.setVgpr(2, 0, 10); // Offset
  ...
  tryExecute(wave, "v_lshl_add_u32 v3, v0, v1, v2");

  // Check Lane 0
  EXPECT_EQ(wave.getVgpr(3, 0), 30);

  // Check Lane 1
  EXPECT_EQ(wave.getVgpr(3, 1), 3);
}
...
TEST(Instructions, V_READFIRSTLANE_B32) {
  ...
  wave.setExecU64(0xFFFFFFFFFFFFFFFF);
  tryExecute(wave, "v_readfirstlane_b32 s0, v0");
  EXPECT_EQ(wave.getSgpr(0), 100);

  // Case 2: Partial EXEC mask (Lanes 0-3 inactive)
  // First active lane is 4 -> Should read 104
  wave.setExecU64(0xFFFFFFFFFFFFFFF0);
  tryExecute(wave, "v_readfirstlane_b32 s0, v0");
  EXPECT_EQ(wave.getSgpr(0), 104);

  // Case 3: Single lane active (Lane 63)
  // Should read 163
  wave.setExecU64(0x8000000000000000);
  tryExecute(wave, "v_readfirstlane_b32 s0, v0");
  EXPECT_EQ(wave.getSgpr(0), 163);
}
...
void runVectorConvertTest(Wave &wave, std::string opName, InT inputVal,
                          OutT expectedVal) {
  // 1. Setup Input (Handle 32-bit vs 64-bit registers)
  if constexpr (sizeof(InT) == 8) {
    wave.setVgpr64(0, 0, std::bit_cast<uint64_t>(inputVal));
  } else {
    wave.setVgpr(0, 0, std::bit_cast<uint32_t>(inputVal));
  }
  ...
  tryExecute(wave, asmLine);

  // 3. Verify Output
  OutT actual;
  if constexpr (sizeof(OutT) == 8) {
    uint64_t raw = wave.getVgpr64(2, 0);
    actual = std::bit_cast<OutT>(raw);
  } else {
    uint32_t raw = wave.getVgpr(2, 0);
    actual = std::bit_cast<OutT>(raw);
  }
...
TEST(Instructions, CVT_F32_U32_FixVerification) {
  ...
  runVectorConvertTest<uint32_t, float>(wave, "v_cvt_f32_u32", 16, 16.0f);

  // Edge case: 0 -> 0.0f
  runVectorConvertTest<uint32_t, float>(wave, "v_cvt_f32_u32", 0, 0.0f);

  // Edge case: Large Integer
  // 16777215 is the largest integer exactly representable in float without gaps
  runVectorConvertTest<uint32_t, float>(wave, "v_cvt_f32_u32", 16777215,
                                        16777215.0f);
}
...
TEST(Instructions, CVT_U32_F32) {
  ...
  // Standard cases
  runVectorConvertTest<float, uint32_t>(wave, "v_cvt_u32_f32", 16.0f, 16);
  runVectorConvertTest<float, uint32_t>(wave, "v_cvt_u32_f32", 0.0f, 0);

  // Truncation check (standard C++ behavior for static_cast)
  // 16.9 -> 16
  runVectorConvertTest<float, uint32_t>(wave, "v_cvt_u32_f32", 16.9f, 16);
}
...
TEST(Instructions, CVT_DoublePrecision) {
  ...
  runVectorConvertTest<uint32_t, double>(wave, "v_cvt_f64_u32", 1, 1.0);
  runVectorConvertTest<uint32_t, double>(wave, "v_cvt_f64_u32", 0xFFFFFFFF,
                                         4294967295.0);

  // v_cvt_u32_f64 (double -> u32)
  runVectorConvertTest<double, uint32_t>(wave, "v_cvt_u32_f64", 12345.6789,
                                         12345);

  // v_cvt_f64_f32 (float -> double)
  runVectorConvertTest<float, double>(wave, "v_cvt_f64_f32", 1.5f, 1.5);

  // v_cvt_f32_f64 (double -> float)
  runVectorConvertTest<double, float>(wave, "v_cvt_f32_f64", 1.5, 1.5f);
}
...
TEST(Instructions, S_MOVK_I32_SignExtension) {
  ...
  tryExecute(wave, "s_movk_i32 s1, 0xFFFF");
  EXPECT_EQ(wave.getSgpr(1), 0xFFFFFFFF);

  // Case 2: Positive 1
  tryExecute(wave, "s_movk_i32 s1, 1");
  EXPECT_EQ(wave.getSgpr(1), 1);

  // Case 3: Positive Limit (0x7FFF = 32767)
  tryExecute(wave, "s_movk_i32 s1, 0x7FFF");
  EXPECT_EQ(wave.getSgpr(1), 32767);

  // Case 4: Negative Limit (0x8000 = -32768 in 16-bit signed)
  // Result should be sign-extended to 0xFFFF8000
  tryExecute(wave, "s_movk_i32 s1, 0x8000");
  EXPECT_EQ(wave.getSgpr(1), 0xFFFF8000);
}
...
TEST(Instructions, MOV_Ops) {
  ...
  // v_mov_b32
  runVectorConvertTest<uint32_t, uint32_t>(wave, "v_mov_b32", 0xDEADBEEF,
                                           0xDEADBEEF);

  // v_mov_b64 (Testing 64-bit vector move)
  runVectorConvertTest<uint64_t, uint64_t>(
      wave, "v_mov_b64", 0xCAFEBABE12345678, 0xCAFEBABE12345678);
}

TEST(Instructions, VMadU64U32) {
  Workgroup wg({.vgprCount = 10, .sgprCount = 10, .waveSize = WaveSize{10}});
  auto &wave = wg.getWave(0);

  // Case 1: Simple multiply-add, no overflow
  // 3 * 5 + 10 = 25
  wave.setVgpr(0, 0, 3);    // S0 = 3
  wave.setVgpr(1, 0, 5);    // S1 = 5
  wave.setVgpr64(2, 0, 10); // S2 = 10
  wave.setSgpr64(0, 0);     // Clear carry destination
  tryExecute(wave, "v_mad_u64_u32 v[4:5], s[0:1], v0, v1, v[2:3]");
  EXPECT_EQ(wave.getVgpr64(4, 0), 25ULL);
  // No overflow, so carry bit for lane 0 should be 0
  EXPECT_EQ(wave.getSgpr64(0) & 1ULL, 0ULL);

  // Case 2: Large multiply that produces a 64-bit result, no 65-bit overflow
  // 0xFFFFFFFF * 0xFFFFFFFF + 0 = 0xFFFFFFFE00000001
  wave.setVgpr(0, 0, 0xFFFFFFFF);
  wave.setVgpr(1, 0, 0xFFFFFFFF);
  wave.setVgpr64(2, 0, 0);
  wave.setSgpr64(0, 0);
  tryExecute(wave, "v_mad_u64_u32 v[4:5], s[0:1], v0, v1, v[2:3]");
  EXPECT_EQ(wave.getVgpr64(4, 0), 0xFFFFFFFE00000001ULL);
  EXPECT_EQ(wave.getSgpr64(0) & 1ULL, 0ULL);
  ...
  wave.setVgpr(0, 0, 0xFFFFFFFF);
  wave.setVgpr(1, 0, 0xFFFFFFFF);
  wave.setVgpr64(2, 0, 0xFFFFFFFFFFFFFFFFULL);
  wave.setSgpr64(0, 0);
  tryExecute(wave, "v_mad_u64_u32 v[4:5], s[0:1], v0, v1, v[2:3]");
  EXPECT_EQ(wave.getVgpr64(4, 0), 0xFFFFFFFE00000000ULL);
  EXPECT_EQ(wave.getSgpr64(0) & 1ULL, 1ULL);

  // Case 4: Literal 0 as src2 (matches the GEMM assembly pattern)
  // v_mad_u64_u32 v[4:5], s[0:1], v0, v1, 0
  wave.setVgpr(0, 0, 100);
  wave.setVgpr(1, 0, 200);
  wave.setSgpr64(0, 0);
  tryExecute(wave, "v_mad_u64_u32 v[4:5], s[0:1], v0, v1, 0");
  EXPECT_EQ(wave.getVgpr64(4, 0), 20000ULL);
  EXPECT_EQ(wave.getSgpr64(0) & 1ULL, 0ULL);

  // Case 5: Multi-lane — verify carry mask is per-lane
  // Lane 0: no overflow, Lane 1: overflow
  wave.setExecU64(0x3); // lanes 0 and 1 active
  wave.setVgpr(0, 0, 2);
  wave.setVgpr(0, 1, 0xFFFFFFFF);
  wave.setVgpr(1, 0, 3);
  wave.setVgpr(1, 1, 0xFFFFFFFF);
  wave.setVgpr64(2, 0, 0);
  wave.setVgpr64(2, 1, 0xFFFFFFFFFFFFFFFFULL);
  wave.setSgpr64(0, 0);
  tryExecute(wave, "v_mad_u64_u32 v[4:5], s[0:1], v0, v1, v[2:3]");
  // Lane 0: 2*3+0 = 6, no carry
  EXPECT_EQ(wave.getVgpr64(4, 0), 6ULL);
  // Lane 1: 0xFFFFFFFF*0xFFFFFFFF + 0xFFFFFFFFFFFFFFFF, carry = 1
  EXPECT_EQ(wave.getVgpr64(4, 1), 0xFFFFFFFE00000000ULL);
  uint64_t carryMask = wave.getSgpr64(0);
...
TEST(Instructions, VccU32U64Consistency) {
  ...
  Workgroup wg64({.vgprCount = 1, .sgprCount = 0, .waveSize = WaveSize{64}});
  auto &regs64 = wg64.getWave(0);
  regs64.setVccU64(0xDEADBEEFCAFEBABEULL);
  EXPECT_EQ(regs64.getVccU64(), 0xDEADBEEFCAFEBABEULL);

  Workgroup wg32({.vgprCount = 1, .sgprCount = 0, .waveSize = WaveSize{32}});
  auto &regs32 = wg32.getWave(0);
...
TEST(Instructions, V_ADD_CO_CI_U32_Wave32) {
  Workgroup wg({.vgprCount = 3, .sgprCount = 0, .waveSize = WaveSize{32}});
  auto &wave = wg.getWave(0);
  ...
  wave.setVccU32(0b110);

  // v_add_co_ci_u32_e32 v2, vcc_lo, v0, v1, vcc_lo
  tryExecute(wave, "v_add_co_ci_u32 v2, vcc_lo, v0, v1, vcc_lo");

  EXPECT_EQ(wave.getVgpr(2, 0), 30u); // 10 + 20 + 0
  EXPECT_EQ(wave.getVgpr(2, 1), 31u); // 10 + 20 + 1
  EXPECT_EQ(wave.getVgpr(2, 2), 0u);  // 0xFFFFFFFF + 0 + 1 = 0 (overflow)

  // Carry-out: only lane 2 overflowed
  EXPECT_EQ(wave.getVccU32() & 0x7, 0b100u);
}
...
TEST(Instructions, V_ADD_CO_U32_Wave32) {
  ...
  wave.setVccU32(0);
  tryExecute(wave, "v_add_co_u32 v2, vcc_lo, v0, v1");

  EXPECT_EQ(wave.getVgpr(2, 0), 0u);  // overflow
  EXPECT_EQ(wave.getVgpr(2, 1), 15u); // no overflow
  // Lane 0 carries, lane 1 does not
  EXPECT_EQ(wave.getVccU32() & 0x3, 0b01u);
}
...
TEST(Instructions, V_MAD_U64_U32_NullCarry_Wave32) {
  Workgroup wg({.vgprCount = 6, .sgprCount = 2, .waveSize = WaveSize{32}});
  auto &wave = wg.getWave(0);

  // Lane 0: 3 * 4 + 100 = 112
  wave.setVgpr(0, 0, 3);
  wave.setVgpr(1, 0, 4);
  wave.setVgpr64(2, 0, 100);

  tryExecute(wave, "v_mad_u64_u32 v[4:5], null, v0, v1, v[2:3]");

  EXPECT_EQ(wave.getVgpr64(4, 0), 112ULL);
}
...
TEST(Instructions, SchedulingHintNoOps_Wave32) {
  Workgroup wg({.vgprCount = 2, .sgprCount = 4, .waveSize = WaveSize{32}});
  auto &wave = wg.getWave(0);

  wave.setVgpr(0, 0, 42);

  // These should not modify any state
  tryExecute(wave, "s_clause 0x1");
  tryExecute(wave, "s_delay_alu instid0(SALU_CYCLE_1) | instskip(NEXT) | "
                   "instid1(VALU_DEP_1)");
...
TEST(Instructions, V_CVT_F32_I32) {
  Workgroup wg({.vgprCount = 2, .sgprCount = 0, .waveSize = WaveSize{32}});
  auto &wave = wg.getWave(0);

  wave.setVgpr(0, 0, 42);
  tryExecute(wave, "v_cvt_f32_i32 v1, v0");
  EXPECT_EQ(std::bit_cast<float>(wave.getVgpr(1, 0)), 42.0f);

  // Negative value
  wave.setVgpr(0, 0, std::bit_cast<uint32_t>(int32_t(-7)));
  tryExecute(wave, "v_cvt_f32_i32 v1, v0");
  EXPECT_EQ(std::bit_cast<float>(wave.getVgpr(1, 0)), -7.0f);
}
...
TEST(Instructions, V_MAD_U32_U24) {
  Workgroup wg({.vgprCount = 4, .sgprCount = 0, .waveSize = WaveSize{32}});
  auto &wave = wg.getWave(0);
  ...
  wave.setVgpr(1, 0, 4);
  wave.setVgpr(2, 0, 10);
  tryExecute(wave, "v_mad_u32_u24 v3, v0, v1, v2");
  EXPECT_EQ(wave.getVgpr(3, 0), 22u);

  // High bits beyond [23:0] should be masked off.
  // 0xFF000003 & 0xFFFFFF = 3, 0xFF000004 & 0xFFFFFF = 4
  wave.setVgpr(0, 0, 0xFF000003);
  wave.setVgpr(1, 0, 0xFF000004);
  wave.setVgpr(2, 0, 0);
  tryExecute(wave, "v_mad_u32_u24 v3, v0, v1, v2");
  EXPECT_EQ(wave.getVgpr(3, 0), 12u);
}
...
TEST(Instructions, ExecLo_Register) {
  Workgroup wg({.vgprCount = 1, .sgprCount = 2, .waveSize = WaveSize{32}});
  auto &wave = wg.getWave(0);

  // exec is initialized to all-ones. On wave-32, exec_lo = 0xFFFFFFFF.
  tryExecute(wave, "s_mov_b32 s0, exec_lo");
  EXPECT_EQ(wave.getSgpr(0), 0xFFFFFFFFu);
}
...
TEST(Instructions, V_CMPX_GT_I32) {
  ...
  tryExecute(wave, "v_cmpx_gt_i32 vcc, v0, v1");
  EXPECT_EQ(wave.getExecU64() & 0x7, 0b001ULL);
}
...
TEST(Instructions, V_CMPX_GT_I32_ImplicitExec) {
  ...
  tryExecute(wave, "v_cmpx_gt_i32 v0, v1");
  EXPECT_EQ(wave.getExecU64() & 0x3, 0b01ULL);
}
...
TEST(Instructions, V_ADD_NC_U32) {
  ...
  wave.setVgpr(0, 0, 10);
  wave.setVgpr(1, 0, 7);
  tryExecute(wave, "v_add_nc_u32 v2, v0, v1");
  EXPECT_EQ(wave.getVgpr(2, 0), 17u);
}
...
TEST(Instructions, V_ADD_NC_I32) {
  ...
  wave.setVgpr(0, 0, 10);
  wave.setVgpr(1, 0, 7);
  tryExecute(wave, "v_add_nc_i32 v2, v0, v1");
  EXPECT_EQ(wave.getVgpr(2, 0), 17u);

  // Negative: 5 + (-3) = 2 (using two's complement)
  wave.setVgpr(0, 0, 5);
  wave.setVgpr(1, 0, static_cast<uint32_t>(-3));
  tryExecute(wave, "v_add_nc_i32 v2, v0, v1");
  EXPECT_EQ(wave.getVgpr(2, 0), 2u);
}
...
TEST(Instructions, V_CMP_EQ_I32) {
  ...
  wave.setVgpr(0, 0, 42);
  wave.setVgpr(1, 0, 42);
  tryExecute(wave, "v_cmp_eq_i32 vcc, v0, v1");
  EXPECT_EQ(wave.getVccU32() & 1, 1u); // lane 0 set

  // Lane 0: not equal
  wave.setVgpr(1, 0, 43);
  tryExecute(wave, "v_cmp_eq_i32 vcc, v0, v1");
  EXPECT_EQ(wave.getVccU32() & 1, 0u); // lane 0 clear

  // Negative values: -1 == -1
  wave.setVgpr(0, 0, static_cast<uint32_t>(-1));
  wave.setVgpr(1, 0, static_cast<uint32_t>(-1));
  tryExecute(wave, "v_cmp_eq_i32 vcc, v0, v1");
  EXPECT_EQ(wave.getVccU32() & 1, 1u);
}
...
TEST(Instructions, V_ALIGNBIT_B32) {
  ...
  // 0xECAFEBAB
  wave.setVgpr(0, 0, 0xDEADBEEF); // high
  wave.setVgpr(1, 0, 0xCAFEBABE); // low
  wave.setVgpr(2, 0, 4);          // shift
  tryExecute(wave, "v_alignbit_b32 v3, v0, v1, v2");
  EXPECT_EQ(wave.getVgpr(3, 0), 0xFCAFEBABu);

  // Shift by 1: ({S0, S1} >> 1)[31:0]
  wave.setVgpr(2, 0, 1);
  tryExecute(wave, "v_alignbit_b32 v3, v0, v1, v2");
  EXPECT_EQ(wave.getVgpr(3, 0), 0xE57F5D5Fu);
}
...
TEST(Instructions, VOPD_DualIssue) {
  ...
  tryExecute(wave, "v_dual_mov_b32 v1, 0 :: v_dual_lshlrev_b32 v2, 2, v0");
  EXPECT_EQ(wave.getVgpr(1, 0), 0u);
  EXPECT_EQ(wave.getVgpr(2, 0), 28u);
}
...
TEST(Instructions, VOPD_DualIssue_PCAdvancement) {
  ...
  tryExecute(wave, "v_dual_mov_b32 v1, 42 :: v_dual_mov_b32 v2, 0");
  int pcAfter = wave.getPc();

  EXPECT_EQ(wave.getVgpr(1, 0), 42u);
  EXPECT_EQ(wave.getVgpr(2, 0), 0u);
...
TEST(Instructions, VccHi_Register) {
  Workgroup wg({.vgprCount = 1, .sgprCount = 2, .waveSize = WaveSize{64}});
  auto &wave = wg.getWave(0);

  // Write a known value to vcc_hi via s_mov_b32
  tryExecute(wave, "s_mov_b32 vcc_hi, 0xABCD0000");
  // Read vcc_hi back into an SGPR
  tryExecute(wave, "s_mov_b32 s0, vcc_hi");
  EXPECT_EQ(wave.getSgpr(0), 0xABCD0000u);
}
...
TEST(Instructions, ExecHi_Register) {
  ...
  tryExecute(wave, "s_mov_b32 s0, exec_hi");
  EXPECT_EQ(wave.getSgpr(0), 0xFFFFFFFFu);

  // Write a new value to exec_hi
  tryExecute(wave, "s_mov_b32 exec_hi, 0x12345678");
  tryExecute(wave, "s_mov_b32 s1, exec_hi");
  EXPECT_EQ(wave.getSgpr(1), 0x12345678u);
}
...
TEST(Instructions, WMMA_F32_16x16x16_BF16) {
  Workgroup wg({.vgprCount = 24, .sgprCount = 0, .waveSize = WaveSize{32}});
  auto &wave = wg.getWave(0);
  ...
  for (int lane = 0; lane < 32; ++lane) {
    ...
    for (int v = 0; v < 8; ++v) {
      ...
      wave.setVgpr(8 + v, lane, lo | (static_cast<uint32_t>(hi) << 16));
    }
  }
  ...
  for (int lane = 0; lane < 32; ++lane) {
    ...
    for (int v = 0; v < 8; ++v) {
      ...
      wave.setVgpr(16 + v, lane, lo | (static_cast<uint32_t>(hi) << 16));
    }
  }
  ...
  for (int lane = 0; lane < 32; ++lane) {
    for (int v = 0; v < 8; ++v) {
      ...
      int col = lane % 16;
      float result = std::bit_cast<float>(wave.getVgpr(v, lane));
...
TEST(Instructions, VPermB32_ByteSelect) {
  ...
  wave.setVgpr(0, 0, 0xDDCCBBAA); // src0
  wave.setVgpr(1, 0, 0x44332211); // src1

  // Selector: pick in[0], in[1], in[2], in[3] → 0x44332211
  wave.setSgpr(0, 0x03020100);
  tryExecute(wave, "v_perm_b32 v2, v0, v1, s0");
  EXPECT_EQ(wave.getVgpr(2, 0), 0x44332211u);

  // Selector: pick in[4], in[5], in[6], in[7] → 0xDDCCBBAA
  wave.setSgpr(0, 0x07060504);
  tryExecute(wave, "v_perm_b32 v2, v0, v1, s0");
  EXPECT_EQ(wave.getVgpr(2, 0), 0xDDCCBBAA);

  // Selector: reverse bytes of src1 → in[3], in[2], in[1], in[0]
  wave.setSgpr(0, 0x00010203);
  tryExecute(wave, "v_perm_b32 v2, v0, v1, s0");
  EXPECT_EQ(wave.getVgpr(2, 0), 0x11223344u);

  // Selector: interleave → in[0], in[4], in[1], in[5]
  wave.setSgpr(0, 0x05010400);
  tryExecute(wave, "v_perm_b32 v2, v0, v1, s0");
  EXPECT_EQ(wave.getVgpr(2, 0), 0xBB22AA11u);
}

TEST(Instructions, VPermB32_Constants) {
  Workgroup wg({.vgprCount = 4, .sgprCount = 4, .waveSize = WaveSize{4}});
  auto &wave = wg.getWave(0);
  wave.setVgpr(0, 0, 0xDDCCBBAA);
  wave.setVgpr(1, 0, 0x44332211);

  // Selector 12 = 0x00, selector 13 = 0xFF
  wave.setSgpr(0, 0x0D0C0D0C);
  tryExecute(wave, "v_perm_b32 v2, v0, v1, s0");
  EXPECT_EQ(wave.getVgpr(2, 0), 0xFF00FF00u);

  // Selector >= 13 all return 0xFF (e.g. 14, 255)
  wave.setSgpr(0, 0xFF0E0D0C);
  tryExecute(wave, "v_perm_b32 v2, v0, v1, s0");
  EXPECT_EQ(wave.getVgpr(2, 0), 0xFFFFFF00u);
}
...
TEST(Instructions, VPermB32_SignExtend) {
  ...
  wave.setVgpr(0, 0, 0x00800000); // src0
  wave.setVgpr(1, 0, 0x00FF0080); // src1
  ...
  wave.setSgpr(0, 0x0B0A0908);
  tryExecute(wave, "v_perm_b32 v2, v0, v1, s0");
  EXPECT_EQ(wave.getVgpr(2, 0), 0x00000000u);

  // Now with src1 = 0x0080FF80 so in[1]=0xFF (bit 7=1), in[3]=0x00
  // src0 = 0x0080FF00 so in[5]=0xFF (bit 7=1), in[7]=0x00
  wave.setVgpr(0, 0, 0x0080FF00); // in[4]=0x00,in[5]=0xFF,in[6]=0x80,in[7]=0x00
  wave.setVgpr(1, 0, 0x0080FF80); // in[0]=0x80,in[1]=0xFF,in[2]=0x80,in[3]=0x00
  ...
  wave.setSgpr(0, 0x0B0A0908);
  tryExecute(wave, "v_perm_b32 v2, v0, v1, s0");
  EXPECT_EQ(wave.getVgpr(2, 0), 0x00FF00FFu);
}

TEST(Instructions, V_EXP_F32) {
  Workgroup wg({.vgprCount = 4, .sgprCount = 4, .waveSize = WaveSize{4}});
  auto &wave = wg.getWave(0);
  wave.setVgpr(0, 0, std::bit_cast<uint32_t>(0.0f));
  wave.setVgpr(0, 1, std::bit_cast<uint32_t>(1.0f));
  wave.setVgpr(0, 2, std::bit_cast<uint32_t>(-1.0f));
  wave.setVgpr(0, 3, std::bit_cast<uint32_t>(2.0f));
  tryExecute(wave, "v_exp_f32 v1, v0");
  // exp2(0) = 1, exp2(1) = 2, exp2(-1) = 0.5, exp2(2) = 4
  EXPECT_FLOAT_EQ(std::bit_cast<float>(wave.getVgpr(1, 0)), 1.0f);
  EXPECT_FLOAT_EQ(std::bit_cast<float>(wave.getVgpr(1, 1)), 2.0f);
  EXPECT_FLOAT_EQ(std::bit_cast<float>(wave.getVgpr(1, 2)), 0.5f);
  EXPECT_FLOAT_EQ(std::bit_cast<float>(wave.getVgpr(1, 3)), 4.0f);
}

TEST(Instructions, V_RCP_F32) {
  Workgroup wg({.vgprCount = 4, .sgprCount = 4, .waveSize = WaveSize{4}});
  auto &wave = wg.getWave(0);
  wave.setVgpr(0, 0, std::bit_cast<uint32_t>(1.0f));
  wave.setVgpr(0, 1, std::bit_cast<uint32_t>(2.0f));
  wave.setVgpr(0, 2, std::bit_cast<uint32_t>(0.5f));
  wave.setVgpr(0, 3, std::bit_cast<uint32_t>(-4.0f));
  tryExecute(wave, "v_rcp_f32 v1, v0");
  EXPECT_FLOAT_EQ(std::bit_cast<float>(wave.getVgpr(1, 0)), 1.0f);
  EXPECT_FLOAT_EQ(std::bit_cast<float>(wave.getVgpr(1, 1)), 0.5f);
  EXPECT_FLOAT_EQ(std::bit_cast<float>(wave.getVgpr(1, 2)), 2.0f);
  EXPECT_FLOAT_EQ(std::bit_cast<float>(wave.getVgpr(1, 3)), -0.25f);
  ...
  tryExecute(wave, "v_rcp_f32 v1, v0");
  EXPECT_TRUE(std::isinf(std::bit_cast<float>(wave.getVgpr(1, 0))));
  EXPECT_EQ(std::bit_cast<float>(wave.getVgpr(1, 1)), 0.0f);
}
...
TEST(Instructions, V_MAX_F32) {
  ...
  wave.setVgpr(0, 0, std::bit_cast<uint32_t>(1.0f));
  wave.setVgpr(1, 0, std::bit_cast<uint32_t>(2.0f));
  wave.setVgpr(0, 1, std::bit_cast<uint32_t>(-3.0f));
  wave.setVgpr(1, 1, std::bit_cast<uint32_t>(-1.0f));
  tryExecute(wave, "v_max_f32 v2, v0, v1");
  EXPECT_FLOAT_EQ(std::bit_cast<float>(wave.getVgpr(2, 0)), 2.0f);
  EXPECT_FLOAT_EQ(std::bit_cast<float>(wave.getVgpr(2, 1)), -1.0f);
}
...
TEST(Instructions, V_MIN_F32) {
  ...
  wave.setVgpr(0, 1, std::bit_cast<uint32_t>(-3.0f));
  wave.setVgpr(1, 1, std::bit_cast<uint32_t>(-1.0f));
  tryExecute(wave, "v_min_f32 v2, v0, v1");
  EXPECT_FLOAT_EQ(std::bit_cast<float>(wave.getVgpr(2, 0)), 1.0f);
  EXPECT_FLOAT_EQ(std::bit_cast<float>(wave.getVgpr(2, 1)), -3.0f);
}
...
TEST(Instructions, V_FMA_F32) {
  ...
  wave.setVgpr(0, 0, std::bit_cast<uint32_t>(2.0f));
  wave.setVgpr(1, 0, std::bit_cast<uint32_t>(3.0f));
  wave.setVgpr(2, 0, std::bit_cast<uint32_t>(10.0f));
  tryExecute(wave, "v_fma_f32 v3, v0, v1, v2");
  EXPECT_FLOAT_EQ(std::bit_cast<float>(wave.getVgpr(3, 0)), 16.0f);

  // -1.5 * 4.0 + 7.0 = 1.0
  wave.setVgpr(0, 0, std::bit_cast<uint32_t>(-1.5f));
  wave.setVgpr(1, 0, std::bit_cast<uint32_t>(4.0f));
  wave.setVgpr(2, 0, std::bit_cast<uint32_t>(7.0f));
  tryExecute(wave, "v_fma_f32 v3, v0, v1, v2");
  EXPECT_FLOAT_EQ(std::bit_cast<float>(wave.getVgpr(3, 0)), 1.0f);
}
...
TEST(Instructions, V_CVT_U32_F32_Clamping) {
  ...
  // Normal
  wave.setVgpr(0, 2, std::bit_cast<uint32_t>(42.7f));
  // Zero
  wave.setVgpr(0, 3, std::bit_cast<uint32_t>(0.0f));
  tryExecute(wave, "v_cvt_u32_f32 v1, v0");
  EXPECT_EQ(wave.getVgpr(1, 0), 0u);
  EXPECT_EQ(wave.getVgpr(1, 1), 0u);
  EXPECT_EQ(wave.getVgpr(1, 2), 42u);
  EXPECT_EQ(wave.getVgpr(1, 3), 0u);
}
...
TEST(Instructions, VPackB32F16) {
  ...
  wave.setVgpr(0, 0, f16_one);
  wave.setVgpr(1, 0, f16_neg2);

  tryExecute(wave, "v_pack_b32_f16 v2, v0, v1");

  uint32_t packed = wave.getVgpr(2, 0);
  ...
  wave.setVgpr(0, 1, 0xDEAD0000u | f16_one);
  wave.setVgpr(1, 1, 0xBEEF0000u | f16_neg2);
  tryExecute(wave, "v_pack_b32_f16 v2, v0, v1");
  packed = wave.getVgpr(2, 1);
  EXPECT_EQ(packed & 0xFFFF, f16_one);
  EXPECT_EQ(packed >> 16, f16_neg2);
}
...
TEST(Instructions, Mfma_F32_16x16x32_F16) {
  ...
  Workgroup wg({.vgprCount = 20, .sgprCount = 4, .waveSize = WaveSize{64}});
  auto &wave = wg.getWave(0);
